define([
	'text!modules/iom/idciom/order/templates/OrderMenegementView.html'+codeVerP,
	'i18n!modules/iom/idciom/order/i18n/order.i18n',
	'modules/common/cloud-utils',
	'css!modules/iom/idciom/order/styles/ordermanagement.css'+codeVerP
	...
	return fish.View.extend({
		...
		_render: function() {
			...
			/*var tacheNameElm =this.$el.find('#order-tacheName');
			utils.ajax('orderTaskManage','qryAllTacheList').done(function(tacheList){
				tacheNameElm.multiselect('option',{dataTextField:'tacheName',dataValueField:'tacheId',dataSource:tacheList});
				var ids = _.map(tacheList, function(obj){ return obj.tacheId; });
				tacheNameElm.multiselect('value',ids);
			});*/
		...
		_afterRender: function() {
			...
			//环节选择
			$('#order-tacheName').popedit({
				dataTextField :'tacheName',
				dataValueField :'tacheId',
				dialogOption:{
					width: 500,
					height: 500,
					viewOptions:{
				...
				url:"js!modules/iom/idciom/order/views/SelectTacheView"
				//url:"js!modules/idcrm/area/views/SelectAreaView"
			});
			//初始化环节按钮
		...
		getMyAgencyPerData: function(page, rowNum, sortname, sortorder) { //请求服务器获取数据的方法
			...
			if(formValue["order-workOrderState"] != undefined && formValue["order-workOrderState"] != ''){
				var workOrderStates = [];
				workOrderStates[0] =formValue["order-workOrderState"];
				qryConditionDto.workOrderState = workOrderStates;
			}
			if(formValue["order-tacheName"] != undefined && formValue["order-tacheName"] != ''){
				qryConditionDto.tacheIds = formValue["order-tacheName"].split(',');
			}
			if(formValue["order-areaId"] != undefined && formValue["order-areaId"] != ''){
				qryConditionDto.areaIds = formValue["order-areaId"].split(',');
			}
		...
		if(!selections||selections.length <=0){
			...
			var getMyAgencyPerData =$.proxy(this.getMyAgencyPerData,this);
			fish.popupView({url: 'modules/iom/idciom/order/views/FinishWorkOrderView',
				width: "60%",
				viewOption:{
					orders : selections,
				},
		...
		if(!selections||selections.length <=0){
			...
			var getMyAgencyPerData =$.proxy(this.getMyAgencyPerData,this);
			fish.popupView({url: 'modules/iom/idciom/order/views/ReturnWorkOrderView',
				width: "60%",
				viewOption:{
					order : selections[0],
				},
	...
	},resDispatchOper:function(){//资源调度
			var selections = this.$("#order-task-grid").grid("getCheckRows");//返回所有被选中的行
			if(selections.length == 1){
				var selected = selections[0];
				fish.popupView({url: 'modules/idcrm/common/popeditviews/dataCenterRoom/views/SelectDCOrRoomView',
					width: "60%",
					viewOption:{
						areaId : 1,
					},
					callback:function(popup,view){
						popup.result.then(function (ret) {
							var exchId,exchCode,exchName,measureId;
							//{"roomId":300002,"roomName":"中原数据基地6号楼A机房","roomCode":"06A","dataCenterId":11002,"dataCenterName":"中原数据基地","dataCenterCode":"ZYDC"}
							exchId = ret.dataCenterId;
							exchCode = ret.dataCenterCode;
							exchName = ret.dataCenterName;
							measureId = ret.roomId;
							utils.ajax('orderTaskManage','updateExchInfo',selected.id,selected.baseOrderId,exchId,exchCode,exchName,measureId,selected.tacheDefineId,selected.areaId,currentUser.staffId).done(function(result){
								if(result == 'success'){
									fish.info({title:'提示',message:'操作成功！'});
								}else{
									fish.info({title:'提示',message:result});
								}
							});
							//console.log(e);
						},function (e) {
			...
			/*$('#idcrm-dcOrRoomtest-selectByArea').popedit({
				dataTextField :'name',
				dataValueField :'id',
				dialogOption: {
					height: 400,
					width: 500,
					viewOptions:{areaId:1},
				},
				...
				utils.ajax('iomService','qryButtonsByTacheId',selected.tacheDefineId,selected.id).done(function(buttons){
					if(buttons != null && buttons != undefined){
						for(var i = 0;i < buttons.length;i++){
							...
							div.attr('style','margin-left: 5px');
