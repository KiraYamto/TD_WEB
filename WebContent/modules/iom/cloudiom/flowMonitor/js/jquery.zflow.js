...
(function($){
    ...
    zflow.state = {
        ...
        //状态编码：状态描述，状态颜色，是否闪烁
        /*stateConfig: {
            "10D": ["正常处理", color.colorGreen, true],
            "10F": ["按时完成", color.colorBlue, false],
            "FFF": ["未处理", color.colorLOCKED, false],
           // "10I": ["未派发", color.colorYellow, false,]
			"10I": ["未派发", color.colorGray, false],
			"10A": ["超时完成", color.colorABOLOSH, false],
            "10E": ["超时未完成", color.colorRed, false]
        }*/
		stateConfig: {
            "10I": ["未开始", color.colorGray, false],
            "10D": ["正常处理中", "#336699", true],
            "10E": ["超时未完成", "#FF0000", false],
			"10F": ["按时完成", "#99CC00", false],
			"10A": ["超时完成", "#FF66FF", false]
        }
    };
    ...
    zflow.Activity = function(activityXML){
		...
		this.areaId = $(activityXML).attr("areaId");
        this.isRunning = $(activityXML).attr("isRunning");
        this.type = $(activityXML).attr("type");
		this.flowTips = $(activityXML).attr("flowTips");
    ...
    zflow.Transition = function(transitionXML){
		...
		//构造画线属性
		this.getArr = function(x1, y1, x2, y2, size) {
        ...
        this.paint = function(size, tTimes){
            ...
            if (this.lineType == "Control") {
                ...
                if (this.nextParallel != null) {
                    ...
                    if (zflow.state.direction == zflow.constant.DIRECTION.H) {
                        ...
                        this.line = this.paper.path("M" + startX + " " + startY 
							...
							+ "L" + endX + " " + endY);
						this.line = this.paper.path(this.getArr(startX,startY,endX,endY,6));
                    }
                } else {
                    //并行结构折线
					var x1 = startX;
					...
					var x2 = endX;
					var y2 = endY;
					var angle = Raphael.angle(x1, y2, x2, y2);//得到两点之间的角度
					var a45 = Raphael.rad(angle - 45);//角度转换成弧度
					var a45m = Raphael.rad(angle + 45);
					var x2a = x2 + Math.cos(a45) * 6;
					var y2a = y2 + Math.sin(a45) * 6;
					var x2b = x2 + Math.cos(a45m) * 6;
					var y2b = y2 + Math.sin(a45m) * 6;
                    if (zflow.state.direction == zflow.constant.DIRECTION.H && startY != endY) {
                        if (this.startNode.type == "Parallel") {
                            ...
                            this.line = this.paper.path("M" + startX + " " + startY 
								...
								+ "L" + endX + " " + endY +  "L" + x2a + " " + y2a + "M" + x2 + " " + y2 +  "L" +  x2b + " " +  y2b);
				...
				col = zflow.state.stateConfig[this.endNode.state][1];
                this.line.attr({
                    "stroke-width": 2,
                    stroke: "#444",
					stroke:col/*,
    ...
    zflow.initState = function(ele, stateConfig){
        ...
        var rectSize = 6;
        var fontSize = 15;
        paperState.text(leftOffset, topOffset + rectSize / 2, "").attr({
            "font-size": fontSize
        });
        var running = new Array();
        var curLeftOffset = leftOffset + 30;
        for (var key in zflow.state.stateConfig) {
            var r = paperState.circle(curLeftOffset, topOffset, rectSize, rectSize).attr({
            ...
            paperState.text(curLeftOffset - 3, topOffset + rectSize / 2 - 2, zflow.state.stateConfig[key][0]).attr({
                "font-size": fontSize
            });
